...
const homeFallback = `<div id="root"><main style="font-family:system-ui,sans-serif;padding:48px;max-width:920px;margin:auto"><h1>Profesyonel Zemin Silim ve Cila Hizmetleri</h1><p>İstanbul merkezli ZeminSilim; Türkiye genelinde mermer, beton, granit, karo, traverten, paladyen ve mozaik yüzeylerde silim, parlatma ve cila uygulamaları sunar.</p><h2>Uzmanlık alanlarımız</h2><ul>${services.map((service) => `<li><a href="/hizmetler/${service.slug}">${escapeHtml(service.title)}</a> — ${escapeHtml(service.shortDescription)}</li>`).join('')}</ul><p><a href="tel:+905079480834">Ücretsiz keşif: +90 507 948 08 34</a></p></main></div>`;
...
function servicePageHtml(service) {
  ...
  let html = home.replace(/<title>[\s\S]*?<\/title>/i, `<title>${escapeHtml(service.title)} İstanbul | ZeminSilim</title>`);
  html = replaceMeta(html, 'name', 'description', service.metaDescription);
  html = replaceMeta(html, 'property', 'og:title', `${service.title} İstanbul | ZeminSilim`);
  ...
  const schema = {
    '@context': 'https://schema.org', '@graph': [
      { '@type': 'Service', '@id': `${url}#service`, name: service.title, serviceType: service.title, description: service.metaDescription, url, image: `${siteUrl}${service.image}`, provider: { '@id': `${siteUrl}/#business` }, areaServed: [{ '@type': 'City', name: 'İstanbul' }, { '@type': 'Country', name: 'Türkiye' }] },
